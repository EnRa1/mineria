#!/usr/bin/env python3
"""
Pipeline de automatizacion para mineria.com.ar (adaptado desde tecno.ar)
==================================================================================
...
NOTA DE ADAPTACION (tecno.ar -> mineria.com.ar):
Toda la arquitectura (ingesta, scoring, ranking, triangulacion, extraccion,
imagen, redaccion con validacion) es identica al pipeline original. Lo que
cambia es el contenido "de dominio": KEYWORDS, las categorias de scoring,
los sitios de referencia para la cascada de busqueda, los ejemplos que se
le dan a Gemini en los prompts de ranking y redaccion, y el nombre del
sitio. Esas son las secciones marcadas con "# === MINERIA:".
"""
...
SITE_NAME = "mineria.com.ar"  # === MINERIA: antes "tecno.ar"
...
MAX_ITEMS_PER_RUN = 5
MAX_HOURS_OLD = 30  # === MINERIA: la minería publica menos seguido que tech;
                     # 16hs de tecno.ar dejaba el pool muy chico. Ajustar según
                     # volumen real observado en la primera semana de corridas.
...
# === MINERIA: sitios de referencia para la cascada de respaldo (Google
# Custom Search). Reemplaza la lista de medios tech de tecno.ar por medios
# especializados en minería (nacionales + internacionales de peso). Los
# nacionales ya deberían aparecer en feeds.txt, pero conviene tenerlos acá
# también porque la cascada busca sobre el tema puntual de la noticia, no
# solo sobre el feed completo.
SITIOS_REFERENCIA_BUSQUEDA = [
    "mineriaydesarrollo.com",
    "noticiasdemineria.com.ar",
    "noticiasmineras.com.ar",
    "panorama-minero.com",
...
DOMINIOS_EXCLUIDOS = {
    ...
    "youtube.com", "mineria.com.ar", "news.google.com",
    "instagram.com",
}
...
# === MINERIA: filtro rapido de relevancia general (reemplaza el KEYWORDS
# de tecno.ar, que era todo de tecnologia). Cualquier entrada de RSS que no
# contenga ninguna de estas palabras se descarta antes de gastar cuota de
# Gemini en ella.
KEYWORDS = [
    "mineria", "minero", "minera", "mineras", "mineros",
    "litio", "cobre", "oro", "plata", "uranio", "zinc", "plomo", "potasio",
    "exploracion minera", "yacimiento", "proyecto minero", "mina",
    "concesion minera", "cateo", "sondaje", "perforacion",
    "rigi", "regalias mineras", "codigo minero",
    "impacto ambiental", "declaracion de impacto ambiental",
    "carbonato de litio", "salar", "puna",
    "camara minera", "secretaria de mineria", "ministerio de mineria",
    "produccion minera", "exportaciones mineras",
]
...
# ----------------------------------------------------------------------
# UTILIDADES (sin cambios respecto de tecno.ar)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# VALIDACION PROGRAMATICA DE TITULOS (determinista, sin cambios)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# === MINERIA: categorias alineadas a los 14 ejes que definiste para el
# sitio, agrupados en 11 categorias de scoring. Cada una suma puntos base
# + bonus si hay un verbo/evento concreto (confirmacion, no especulacion).

LAUNCH_KEYWORDS = [
    "confirma", "anuncia", "anuncio", "presenta", "presento", "aprueba",
    "aprobo", "firma", "firmo", "inaugura", "arranca", "comienza",
    "lanza", "habilita", "adjudica", "licita", "descubre", "confirma reservas",
    "launches", "announces", "confirms", "approves",
]

LITIO_KEYWORDS = [
    "litio", "carbonato de litio", "hidroxido de litio", "salar",
    "salmuera", "salar del hombre muerto", "fenix", "olaroz", "cauchari",
    "sal de vida", "kachi", "rincon", "tres quebradas",
]

COBRE_KEYWORDS = [
    "cobre", "concentrado de cobre", "taca taca", "mara", "agua rica",
    "josemaria", "los azules", "el pachon", "filo del sol",
]

ORO_PLATA_KEYWORDS = [
    "oro", "plata", "onzas de oro", "onzas de plata", "veta", "cerro leon",
    "cerro vanguardia", "casposo", "gualcamayo", "diablillos",
]

EXPLORACION_KEYWORDS = [
    "exploracion minera", "cateo", "sondaje", "perforacion", "yacimiento",
    "recursos indicados", "recursos inferidos", "estudio de factibilidad",
    "nuevo objetivo de exploracion", "campaña de exploracion",
]

EMPRESAS_INVERSION_KEYWORDS = [
    "inversion minera", "ronda de inversion", "adquiere", "adquisicion",
    "fusion", "socio minoritario", "financiamiento minero", "rigi",
    "millones de dolares", "capex", "ampliacion de la produccion",
]

PROVEEDORES_EQUIPAMIENTO_KEYWORDS = [
    "proveedor minero", "proveedores mineros", "capemisa", "equipamiento minero",
    "maquinaria minera", "camion minero", "perforadora", "pala mecanica",
    "cadena de proveedores", "compre minero",
]

ENERGIA_INFRAESTRUCTURA_KEYWORDS = [
    "energia electrica", "linea de alta tension", "infraestructura minera",
    "energia renovable minera", "planta desaladora", "gasoducto",
    "abastecimiento electrico", "obra electrica",
]

LEGISLACION_KEYWORDS = [
    "codigo minero", "regalias mineras", "declaracion de impacto ambiental",
    "ley minera", "regulacion minera", "licencia ambiental", "rigi",
    "secretaria de mineria", "ministerio de mineria", "politica minera",
]

SUSTENTABILIDAD_KEYWORDS = [
    "sustentabilidad minera", "medio ambiente", "policia ambiental minera",
    "remediacion", "cierre de mina", "huella de carbono", "mineria responsable",
    "gestion del agua", "reciclado de agua", "revegetacion",
]

INNOVACION_DIGITAL_KEYWORDS = [
    "digitalizacion minera", "automatizacion minera", "drones",
    "inteligencia artificial minera", "nanotecnologia", "mineria 4.0",
    "sensores mineros", "software minero", "gemelo digital",
]

EVENTOS_KEYWORDS = [
    "expo minera", "congreso minero", "feria minera", "arminera",
    "perumin", "finnmateria", "jornadas mineras", "encuentro minero",
    "cumbre minera",
]

SEGURIDAD_KEYWORDS = [
    "seguridad minera", "accidente minero", "aoma", "primer respondiente",
    "emergencia minera", "salud ocupacional minera",
]

# === MINERIA: reemplaza el bonus "argentina" de tecno.ar (que ahi tenia
# sentido porque el sitio era global-por-default). Acá el sitio YA es
# argentino por default, así que este bonus se usa al revés: para noticias
# de peso INTERNACIONAL que igual importan porque tienen impacto directo en
# la actividad argentina (precio de commodities, jugadores globales que
# operan en el pais).
IMPACTO_ARGENTINA_KEYWORDS = [
    "argentina", "catamarca", "salta", "jujuy", "san juan", "mendoza",
    "santa cruz", "rio negro", "chubut", "neuquen", "la rioja",
    "ypf litio", "ganfeng", "livent", "arcadium", "glencore", "barrick",
    "mcewen mining", "first quantum", "posco argentina", "lundin mining",
]

PENALTY_KEYWORDS = [
    "lo que tenés que saber", "imperdible", "no te pierdas", "resumen del día",
    "lo mejor de", "top 5", "top 10", "opinion", "columna de opinion",
]
...
def compute_relevance_score(entry_text):
    ...
    grupos = [
        (LITIO_KEYWORDS, "litio"),
        (COBRE_KEYWORDS, "cobre"),
        (ORO_PLATA_KEYWORDS, "oro_plata"),
        (EXPLORACION_KEYWORDS, "exploracion"),
        (EMPRESAS_INVERSION_KEYWORDS, "empresas_inversion"),
        (PROVEEDORES_EQUIPAMIENTO_KEYWORDS, "proveedores_equipamiento"),
        (ENERGIA_INFRAESTRUCTURA_KEYWORDS, "energia_infraestructura"),
        (LEGISLACION_KEYWORDS, "legislacion"),
        (SUSTENTABILIDAD_KEYWORDS, "sustentabilidad"),
        (INNOVACION_DIGITAL_KEYWORDS, "innovacion_digital"),
        (EVENTOS_KEYWORDS, "eventos"),
        (SEGURIDAD_KEYWORDS, "seguridad"),
    ]

    for kw_list, nombre in grupos:
    ...
    if any(kw in text for kw in IMPACTO_ARGENTINA_KEYWORDS):
        score += 1
        categorias.append("impacto_argentina")
...
# (sin cambios de logica; el prompt es generico y funciona igual para
# cualquier dominio, solo cambia el contenido de item['title']/summary)
# ----------------------------------------------------------------------
...
def buscar_fuentes_con_grounding(item, max_fuentes=MAX_FUENTES_ADICIONALES):
    ...
    prompt = f"""
Busca en la web noticias RECIENTES (ultimas 24-72 horas) que cubran el mismo
hecho o tema que esta noticia sobre mineria en Argentina:
...
el tema general, sino cobertura del mismo hecho puntual). Priorizá medios
especializados en mineria (Panorama Minero, Mineria y Desarrollo, Noticias de
Mineria, Mining.com, Mining Weekly) o economicos de referencia (Ambito,
Infobae, iProfesional, BNamericas) por sobre medios generalistas.
...
# ----------------------------------------------------------------------
# BUSQUEDA DE IMAGEN VIA GOOGLE CUSTOM SEARCH (sin cambios)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# EXTRACCION DEL ARTICULO COMPLETO (sin cambios)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# HELPER COMPARTIDO: LLAMADA A GEMINI CON RETRY/BACKOFF (sin cambios)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# FILTRO CONTEXTUAL CON GEMINI (ranking) - prompt adaptado a mineria
# ----------------------------------------------------------------------
...
def rank_with_gemini(candidatos):
    ...
    # === MINERIA: prompt de ranking reescrito. Se mantiene la misma
    # estructura de 4 ejes de tecno.ar (confirmacion, impacto, actualidad,
    # valor informativo) porque es agnostica al dominio, pero se
    # reemplazan los ejemplos por casos reales de mineria y las categorias
    # por las 12 que definimos para mineria.com.ar.
    prompt = f"""
Eres un editor jefe de mineria.com.ar, un medio especializado en la
industria minera de Argentina. Tu tarea es ORDENAR POR RELEVANCIA REAL las
{pool_objetivo} noticias más importantes de la lista al final de este mensaje.
...
===========================================
mineria.com.ar cubre Litio, Cobre, Oro y Plata, Exploración minera,
Empresas e Inversiones, Proveedores y Equipamiento, Energía e
Infraestructura, Legislación minera, Sustentabilidad y medio ambiente,
Innovación y transformación digital, Eventos del sector, y Seguridad
minera. Estas categorías son TODAS de igual jerarquía de partida. NO existe
una tabla de prioridad fija donde "litio" le gane automáticamente a
"legislación" o viceversa. Una nueva regulación provincial que frena un
proyecto, un accidente minero real, o una inversión multimillonaria
confirmada DEBEN poder superar en el ranking a un anuncio de exploración
menor, si su impacto real es mayor. Evaluá cada noticia por su propio
mérito periodístico, no por a qué categoría pertenece.
...
Puntuación alta: hecho confirmado oficialmente por la empresa, la provincia
o el organismo involucrado, o evento ya ocurrido y verificable (una
inversión firmada, una declaración de impacto ambiental aprobada, un
hallazgo de reservas confirmado con estudio técnico, una nueva ley o
regalía sancionada, un accidente real).
Puntuación baja o descarte: rumores, "fuentes cercanas al proyecto
afirman", especulación ("se espera que", "podría avanzar"), notas de
opinión, listados genéricos ("lo que hay que saber sobre el litio").

EJE 2 — IMPACTO Y ALCANCE REAL
¿A cuánta gente, cuánta producción o cuánta inversión afecta? Una nueva
regalía provincial que rige para toda una jurisdicción, una inversión de
cientos de millones de dólares, o un accidente con víctimas tienen el
MISMO nivel de impacto que un gran hallazgo de litio — no menos. Un
anuncio menor de una junior poco conocida, o una nota de color sobre un
evento local, tienen impacto bajo, sea cual sea su categoría.

EJE 3 — ACTUALIDAD
Qué tan reciente y puntual es el hecho dentro de la ventana de tiempo
cubierta. Más reciente y más "de hoy" puntúa mejor que algo que ya se
viene arrastrando hace días o que es un seguimiento sin novedad real.

EJE 4 — VALOR INFORMATIVO PARA EL LECTOR
¿Le aporta algo concreto al lector (una cifra de producción, un cambio
normativo que afecta su operación, una fecha de un evento, un riesgo de
seguridad)? Se descarta siempre, sin importar el tema: opinión o análisis
retrospectivo, rankings y listicles, y contenido educativo genérico sin un
hecho puntual detrás.

===========================================
CÓMO SE VE UN EJE 1 ALTO EN CADA CATEGORÍA (ejemplos, no jerarquía)
===========================================
- Litio: "LIEX amplía su producción de litio en Catamarca con una inversión de USD 709 millones"
- Cobre: "First Quantum avanza en la búsqueda de un socio minoritario para Taca Taca"
- Oro/Plata: "Aumentan más del 70% las onzas de plata probadas en Cerro León, Santa Cruz"
- Exploración: "Orestone confirma una nueva zona de vetas de oro en la propiedad Francisca, Salta"
- Empresas/Inversiones: "El Comité Evaluador aprueba la ampliación de LIEX S.A. por USD 709 millones"
- Proveedores/Equipamiento: "CAPEMISA acuerda con el gobierno de Salta medidas para sostener a proveedores mineros"
- Energía/Infraestructura: "Avanza la obra eléctrica para abastecer al proyecto PSJ Cobre Mendocino"
- Legislación: "El Gobierno de Catamarca aprueba la Declaración de Impacto Ambiental del proyecto Diablillos"
- Sustentabilidad: "La Policía Ambiental Minera de San Juan suma drones para fiscalización"
- Innovación/Digital: "La UNSJ abre el primer Laboratorio de Nanotecnología Aplicada a la Minería del país"
- Eventos: "Arranca la Expo Arminera 2026 con récord de expositores"
- Seguridad: "Un accidente en una mina de Santa Cruz deja trabajadores heridos"

Ejemplos de EJE 1 bajo o descarte en cualquier categoría: "Todo lo que hay
que saber sobre el litio argentino" (genérico), "¿Podría Argentina
convertirse en potencia del cobre?" (especulación), "Los 10 proyectos
mineros más importantes de 2026" (listicle), "Fuentes del sector creen que
podría aprobarse la ley" (rumor sin confirmación oficial).
...
(más provincias/empresas/trabajadores afectados pesa más que un caso
aislado), (c) mayor actualidad dentro de la ventana de tiempo. Esto aplica
igual entre dos noticias de la misma categoría o de categorías distintas.
...
# ----------------------------------------------------------------------
# INGESTA + FILTROS (sin cambios de logica)
# ----------------------------------------------------------------------
...
def fetch_new_relevant_items():
    ...
    for it in seleccionados_final:
        print(f"  [{it['score_reglas']:>2}pts | {it['categoria_reglas']:<22}] {it['title'][:70]}")

    return seleccionados_final, candidatos_para_triangular

# ----------------------------------------------------------------------
# CONSTRUCCION DEL PROMPT DE REDACCION
# ----------------------------------------------------------------------
...
===========================================
Estos son los ultimos titulos publicados en mineria.com.ar. Tu nuevo
SEO_TITLE y H1 NO pueden empezar con la misma formula, palabra de apertura,
o estructura sintactica que ninguno de estos, aunque el tema de hoy sea
completamente distinto.

{lista_titulos}
"""

    # === MINERIA: mismo esqueleto de instrucciones SEO que tecno.ar (es
    # agnostico al dominio), pero con "tecno.ar" -> "mineria.com.ar",
    # registro adaptado a un publico tecnico/corporativo del sector
    # (empresarios, inversores, funcionarios, proveedores), y ejemplos de
    # titulo/keyword reemplazados por casos de mineria.
    return f"""Actua como un redactor especializado en periodismo minero y
economico, con dominio experto de los criterios de puntuacion de Rank Math
para WordPress, escribiendo para el sitio mineria.com.ar.
...
que un lector diria en una oracion normal en español. Muchas veces la
noticia tiene un proyecto o empresa con nombre propio (ej: "Taca Taca" de
"First Quantum", "Fénix" de "Livent"). En esos casos, JAMAS encadenes los
dos nombres propios uno al lado del otro sin conector, porque eso no es una
frase real y despues no se puede insertar en el texto sin que quede
forzado.
...
en la oracion, vas a generar una redundancia (sujeto repetido). Por
ejemplo, si el keyword es "inversion de LIEX en Catamarca" y armas la
oracion "LIEX confirmó una inversion de LIEX en Catamarca...", eso es un
ERROR GRAVE: el sujeto "LIEX" aparece dos veces.
...
(proyecto, mina, planta, empresa, provincia). Si armas un titulo o una
oracion que envuelve al keyword repitiendo esa MISMA palabra, se genera
una redundancia. Por ejemplo, con el keyword "proyecto de litio Fénix":
- MAL: "Livent avanza con el proyecto de litio Fénix en su proyecto de Catamarca..."
  (la palabra "proyecto" aparece dos veces)
- BIEN: "Livent confirma una nueva etapa de producción en Fénix, Catamarca"
Otro ejemplo, con el keyword "declaración de impacto ambiental del proyecto Diablillos":
- MAL: "Diablillos: aprueban la declaración de impacto ambiental del proyecto Diablillos"
- BIEN: "Catamarca da luz verde a un nuevo proyecto de oro y plata en la Puna"
...
- "Taca Taca First Quantum"        -> mal: dos nombres propios pegados, no es una frase
- "LIEX Catamarca Inversion"       -> mal: tres sustantivos sin conector
- "First Quantum busca socio"      -> mal: ya es una oracion con sujeto y verbo propios

EJEMPLOS DE KEYWORDS CORRECTOS:
- "inversión de LIEX en Catamarca"                     (sustantivo + complementos)
- "declaración de impacto ambiental del proyecto Diablillos"  (evento como sustantivo)
- "producción de plata en Cerro León"                  (sustantivo + complementos)
...
3. ¿Es asi como lo diria un periodista especializado en mineria en voz alta?
...
===========================================
El titulo es lo unico que decide si alguien entra a leer la nota o la
ignora en el feed. El lector de mineria.com.ar suele ser un profesional del
sector (empresario, inversor, funcionario, proveedor), asi que el titulo
tiene que sonar preciso y verificable, no sensacionalista. Escribilo con
estos principios, en este orden de importancia:
...
1. EMPEZA POR EL HECHO, NO POR UNA INTRODUCCION:
   ...
   MAL: "Todo sobre el nuevo proyecto de litio en Catamarca"
   BIEN: "LIEX invierte USD 709 millones para ampliar su producción de litio en Catamarca"

2. USA UN VERBO FUERTE Y ACTIVO, NUNCA UNO DEBIL O GENERICO:
   Preferi verbos de accion concreta (invierte, confirma, aprueba, firma,
   habilita, descubre, frena, exporta, gana, pierde, demanda, sanciona) por
   sobre verbos vagos (tiene que ver con, esta relacionado a, habla de).

3. INCLUI UN DATO O NUMERO CONCRETO SI LA NOTICIA LO TIENE:
   Un monto de inversion, un porcentaje, una tonelada, o un nombre de
   proyecto ancla el titulo en la realidad y genera mas confianza que una
...
4. CONTENIDO:
   ...
   - Voz activa, tono profesional y riguroso (español rioplatense, pero sin
     informalidad excesiva: el publico de mineria.com.ar es tecnico/corporativo).
   - Cuando cites cifras (inversion, produccion, regalias), sé preciso con
     las unidades (USD, toneladas, onzas, %) tal como figuran en la fuente.
...
# ----------------------------------------------------------------------
# GUARDADO DE BORRADORES LOCALES (sin cambios)
# ----------------------------------------------------------------------
...
def main():
    print(f"🚀 Iniciando pipeline para {SITE_NAME}...")
    ...
    print(f"DEBUG: MAX_ITEMS_PER_RUN = {MAX_ITEMS_PER_RUN} | RANKING_POOL_SIZE = {RANKING_POOL_SIZE} "
          f"| MAX_POR_FUENTE = {MAX_POR_FUENTE} | MAX_HOURS_OLD = {MAX_HOURS_OLD}")
